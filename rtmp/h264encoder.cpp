...
namespace LQF
{
    ...
    int H264Encoder::Init(const Properties& properties)
    {
        ...
        av_image_fill_arrays(frame_->data, frame_->linesize, picture_buf_, ctx_->pix_fmt, ctx_->width, ctx_->height, 1);
    ...
    int H264Encoder::Encode(uint8_t* in, int in_samples, uint8_t* out, int& out_size)
    {
        ...
        // 3. 循环接收编码后的数据包
        //    新 API 是异步的，可能一次 send 需要多次 receive
        //    但为了匹配你旧的 1:1 签名，我们在获取第一个包后立即返回。
        while (true)
        {
            // 在接收包之前，必须先取消引用 (unref) packet_。
            // 这是为了清理由 packet_ 持有的任何先前编码的数据。
            av_packet_unref(&packet_);
            ...
            {
                // AVERROR(EAGAIN): 编码器需要更多输入(即下一帧)才能输出。
                // 这相当于旧 API 中的 (got_picture == 0)。
                // AVERROR_EOF: 编码器已刷新，流结束。
                //
                // 在这两种情况下，这一轮都没有可用的包，
                // 我们返回 -1 (或 0，取决于你的上层逻辑)，
                // 表示"成功发送，但没有包输出"。返回 -1 以匹配你的旧逻辑。
                return -1;
            }
            else if (ret < 0)
            {
            ...
            // 保留你的原始逻辑：跳过 00 00 00 01 startcode nalu
            if (packet_.size > 4)
            {
            ...
            framecnt++; // 假设 framecnt 是成员变量

            // 我们已成功获取并处理了一个包。
            // 为匹配你原始函数的 1:1 行为，我们在这里立即返回 "成功"。
            // packet_ 将在下一次 Encode 调用时在循环顶部被 unref。
            return 0;
        }
    ...
    int H264Encoder::get_sps(uint8_t* sps, int& sps_len) const
    {
    ...
    AVPacket* H264Encoder::Encode(uint8_t* yuv, uint64_t pts, const int flush)
    {
        ...
        const int ret = avcodec_receive_packet(ctx_, packet);
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF)
        {
            return nullptr;
        }
