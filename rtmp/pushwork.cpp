...
namespace LQF
{
    ...
    RET_CODE PushWork::Init(const Properties& properties)
    {
        ...
        av_channel_layout_default(&ch_layout, audio_channels_);
        audio_ch_layout_ = ch_layout.u.mask;
        ...
        aud_params.src_channel_layout = ch_layout.u.mask;
        ...
        auto* metadata = new FLVMetadataMsg();
        // 设置视频相关
        ...
        audio_capturer_->AddCallback([this](uint8_t* pcm, int32_t size)
        {
            this->PcmCallback(pcm, size);
        });
    ...
    void PushWork::PcmCallback(uint8_t* pcm, int32_t size)
    {
        ...
        if (need_send_audio_spec_config)
        {
            ...
            auto* aud_spc_msg = new AudioSpecMsg(audio_encoder_->get_profile(),
        ...
        ret = audio_resampler_->ReceiveResampledFrame(
            resampled_frames,
            static_cast<int>(audio_encoder_->GetFrameSampleSize()));
        ...
        for (auto& resampled_frame : resampled_frames)
        {
            if (!pcm_flt_fp_)
            {
            ...
            // 封装带参考计数的缓存
            int aac_size = audio_encoder_->Encode(resampled_frame.get(),
            ...
            if (aac_size > 0)
            {
                ...
                auto* aud_raw_msg = new AudioRawMsg(aac_size + 2);
                // 打上时间戳
    ...
    void PushWork::YuvCallback(uint8_t* yuv)
    {
        ...
        if (need_send_video_config)
        {
            ...
            auto* vid_config_msg = new VideoSequenceHeaderMsg(
        ...
        {
            // 获取到编码数据
            auto* nalu = new NaluStruct(video_nalu_buf, video_nalu_size_);
