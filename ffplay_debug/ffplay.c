...
/**
 * @brief 封装的 Packet 队列元素类型
 *
 * @param pkt ffmpeg解码器使用的压缩数据包
 * @@param serial 封装的 Packet 队列元素所属的 PacketQueue 的序列号
 */
...
typedef struct PacketQueue {
    ...
    int size;
    int64_t duration;
    int abort_request; // 是否退出 1：退出 0：未退出
    int serial;
    SDL_mutex *mutex;
    SDL_cond *cond;
} PacketQueue;
...
typedef struct VideoState {
    SDL_Thread *read_tid;// 播放控制：读取线程句柄，负责从文件/网络读取数据
    const AVInputFormat *iformat;
    int abort_request;// 播放控制：是否请求退出
    int force_refresh;
    int paused;// 播放控制：是否暂停
    int last_paused;// 播放控制：上一次暂停状态
    int queue_attachments_req;// 播放控制：是否需要附加文件信息
    int seek_req;   // 播放控制：是否需要跳转
    int seek_flags; // 播放控制：跳转标志
    int64_t seek_pos;// 播放控制：跳转位置
    int64_t seek_rel;
    int read_pause_return;
    AVFormatContext *ic;
    int realtime;

    Clock audclk;// 时钟同步：音频时钟
    Clock vidclk;// 时钟同步：视频时钟
    Clock extclk;// 时钟同步：外部时钟

    FrameQueue pictq;//数据队列：视频数据队列
    FrameQueue subpq;//数据队列：字幕数据队列
    FrameQueue sampq;//数据队列：音频数据队列

    Decoder auddec;//解码器相关：音频解码器
    Decoder viddec;//解码器相关：视频解码器
    Decoder subdec;//解码器相关：字幕解码器

    int audio_stream;

    int av_sync_type;// 时钟同步：同步策略（音频主导、视频主导、外部时钟）

    double audio_clock;// 时钟同步：音频时钟
    int audio_clock_serial;// 时钟同步：音频时钟序列号
    double audio_diff_cum; // 时钟同步：音频时钟偏差
    double audio_diff_avg_coef;// 时钟同步：音频时钟偏差平均系数
    double audio_diff_threshold;// 时钟同步：音频时钟偏差阈值
    int audio_diff_avg_count;
    AVStream *audio_st;//解码器相关：音频流
    PacketQueue audioq;//数据队列：音频数据队列
    int audio_hw_buf_size;//音频播放：音频播放缓存大小
    uint8_t *audio_buf;//音频播放：音频播放缓存
    uint8_t *audio_buf1;//音频播放：音频播放缓存
    unsigned int audio_buf_size; /* in bytes */ //音频播放：音频播放缓存大小
    unsigned int audio_buf1_size;//音频播放：音频播放缓存大小
    int audio_buf_index; /* in bytes */ //音频播放：音频播放缓存索引
    int audio_write_buf_size;
    int audio_volume;
    int muted;//音频播放：是否静音 1：静音 0：取消静音
    struct AudioParams audio_src;//音频播放：音频播放源参数
    struct AudioParams audio_filter_src;//音频播放：音频播放源参数
    struct AudioParams audio_tgt;//音频播放：音频播放目标参数
    struct SwrContext *swr_ctx;//音频播放：音频播放重采样上下文
    ...
    SDL_Texture *vis_texture;//视频播放：可视化纹理
    SDL_Texture *sub_texture;//视频播放：字幕纹理
    SDL_Texture *vid_texture;//视频播放：视频纹理

    int subtitle_stream;
    AVStream *subtitle_st;//解码器相关：字幕流
    PacketQueue subtitleq;//数据队列：字幕数据队列

    double frame_timer;
    double frame_last_returned_time;
    double frame_last_filter_delay;
    int video_stream;
    AVStream *video_st;//解码器相关：视频流
    PacketQueue videoq;//数据队列：视频数据队列
    double max_frame_duration;      // maximum duration of a frame - above this, we consider the jump a timestamp discontinuity
    struct SwsContext *sub_convert_ctx;
    int eof;

    char *filename;//文件名
    int width, height, xleft, ytop;//窗口大小
    int step;
    ...
    SDL_cond *continue_read_thread;//播放控制：继续读取线程条件句柄
} VideoState;
 ...
 * 初始化视频/音频/
 *
 * @note FrameQueue 是解码后的帧缓冲区，多线程中用来在解码线程和显示线程之间传递帧
 * @param f 要初始化的 FrameQueue 结构体
 * @param pktq 对应的压缩包队列 PacketQueue（视频/音频/字幕）
 * @param max_size 队列大小
 * @param keep_last 是否保留最后一帧，用于视频/字幕同步显示
 * @return
 */
 ...
 * @param c 时钟结构体
 * @param pts 当前时间戳
 * @param serial 当前PacketQueue序列号，用于判断是否属于当前播放段
 */
static void set_clock(Clock *c, double pts, int serial) {
    double time = av_gettime_relative() / 1000000.0;//返回的是硬件系统启动到目前的时间
 ...
 *
 * @param c 时钟结构体
 * @param queue_serial  只想对应队列的序列号（PacketQueue的serial）
 */
...
/**
 * 音频过滤器配置
 *  1、输入滤镜 (abuffer) → 接收解码器输出的 PCM 数据
 *  2、输出滤镜 (abuffersink) → 将滤镜处理后的 PCM 数据送到 SDL 音频输出
 *  3、配置滤镜参数（采样率、格式、通道布局）
 *  4、根据 afilters 应用用户指定的滤镜链
 * @param is
 * @param afilters
        ...
        if (got_frame) {
            ...
            if (reconfigure) {
                ...
                av_log(NULL, AV_LOG_DEBUG,
                       ...
                       frame->sample_rate, frame->ch_layout.nb_channels, av_get_sample_fmt_name(frame->format), buf2,
...
/**
 * ffplay 的视频解码线程核心，它负责从视频包队列里取出帧，经过滤镜处理，然后放入显示队列等待渲染
 * 从视频队列取帧
 * 检测帧属性变化，重建滤镜图
 * 帧输入滤镜图
 * 帧输入滤镜图
...
/**
 * @details:
 *  1、分配并初始化 AVCodecContext。
 *  2、找到解码器 (AVCodec)。
 *  3、配置解码器参数。
 *  4、初始化滤镜（音频）或队列。
 *  5、启动解码线程。
 * @param is 播放器状态结构体，包含所有流的上下文、队列、解码器等。
 * @param stream_index 你要打开的视频/音频/字幕流的索引。
...
//    将 AVCodecParameters 中存储的流信息（视频/音频/字幕）复制到解码器上下文 AVCodecContext。
...
static int read_thread(void *arg) {
    ...
    /**
     * 会周期调用该函数
     * 1、在读取数据时，每读取一个数据包后会检查中断标志
     * 2、在网络流中，通常每几毫秒到几十毫秒调用一次
     * 3、在文件I/O中，通常在每次系统调用后检查
     */
...
//    只是为了读取某些TS流
    ...
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        ...
        AVRational sar = av_guess_sample_aspect_ratio(ic, st, NULL);//推测视频流或帧的像素纵横比
        if (codecpar->width)
...
//        判断当前 packet (pkt) 是否属于用户设置的播放时间范围 [start_time, start_time+duration]
...
/**
 * 打开一个媒体文件并初始化 VideoState 的入口函数
 * @note: 负责创建一个 VideoState 并初始化它的全部成员。
 * @param filename 媒体文件名
 * @param iformat 媒体文件格式
 * @return VideoState ffplay 的核心数据结构，保存解码器、队列、时钟、音量、流信息等
 */
static VideoState *stream_open(const char *filename,
    ...
    is->muted = 0;
    is->av_sync_type = av_sync_type;//设置同步方式：默认使用声音作为同步方式
    is->read_tid = SDL_CreateThread(read_thread, "read_thread", is);//创建一个线程，用于读取媒体文件,创建后会立刻启动
    if (!is->read_tid) {
...
/**
 * @brief ffplay 主渲染循环的一部分，负责事件处理、视频刷新、鼠标隐藏等
 * @param is 上帝对象
 * @param event sdl时间对象
 */
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
    double remaining_time = 0.0;
    SDL_PumpEvents();//刷新事件，将操作系统的事件（鼠标、键盘、窗口）放入 SDL 内部队列，只收集不处理
//    从事件队列中获取事件
...
int main(int argc, char **argv) {
    ...
    av_log_set_flags(AV_LOG_SKIP_REPEATED);//忽略重复日志
    parse_loglevel(argc, argv, options);//解析日志等级
